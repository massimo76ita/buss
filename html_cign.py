...
STAZIONE = "CIGN"
NOME_COMPLETO = "Sant'Elia a Pianisi (CB)"
COLORE = "#ff6b00"
CANALIZZAZIONE = "HHZ"
...
# === Tempo corrente e filtro eventi ===
...
totali = len(docs)
ultima = docs[-1]["timestamp_cest"] if totali else "—"
orario = datetime.fromisoformat(ultima).strftime("%H:%M:%S CEST") if totali else "—"
...
<div class="gallery">""")
    ...
    for d in docs:
        try:
            ...
            label = datetime.fromisoformat(d["timestamp_cest"]).strftime("%H:%M:%S CEST")
            peak = d.get("peak", "—")
            rms = d.get("rms", "—")
